    ...
    <div class="card text-left">
        <div class="card-header">
            <ul class="nav nav-tabs card-header-tabs">
                ...
                <li class="nav-item">
                    <a class="nav-link active" aria-current="true" href="processos.php?pg=<?php echo $row["page"];?>">Processos SEI</a>
                </li>
                <li class="nav-item">
        ...
        <div class="table-responsive">
            <table class="table table-borderless table-hover">
                ...
                <?php
                    global $pdo;
                    $equipe = $row["equipe"];
                    $sql = "SELECT * FROM processos WHERE equipe = '$equipe'";
                    $sql = $pdo->prepare($sql);
                    $sql->execute();
                    
                    $i = 0;

                    while($proc = $sql -> fetch(PDO::FETCH_ASSOC)):
                        $i++;
                ?>
                <tbody>
                    <tr>
                        <td><?php echo $i;?></td>
                        <td><?php echo $proc["lotacao"];?></td>
                        <td><?php echo $proc["objeto"];?></td>
                        <td>
                            <a href="" onclick="window.open('https://sei.inss.gov.br/sei/controlador.php?acao=procedimento_trabalhar&id_procedimento=<?php echo $proc["idproc"];?>')">
                                <?php echo $proc["processo"];?>
                            </a>
                        </td>
                    </tr>
                </tbody>            
